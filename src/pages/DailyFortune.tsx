...
import { Loader2, RefreshCw, AlertCircle, Sparkles, TrendingUp, Heart, Briefcase, Coins, Book, Users } from 'lucide-react';
...
export const DailyFortune: React.FC = () => {
  ...
  const getScoreLevel = (score: number) => {
    if (score >= 90) return { label: '极佳', color: '#4ade80' };
    if (score >= 75) return { label: '良好', color: '#22c55e' };
    if (score >= 60) return { label: '平稳', color: '#eab308' };
    if (score >= 40) return { label: '注意', color: '#f97316' };
    return { label: '谨慎', color: '#ef4444' };
  };
        ...
        <>
          ...
          >
            ...
            <div className="flex items-center gap-2 text-sm">
              ...
              <span style={{ color: getScoreLevel(averageScore).color }} className="font-medium">
...
const ReadingCard: React.FC<ReadingCardProps> = ({ title, icon, color, text, delay = 0 }) => (
  <motion.div 
