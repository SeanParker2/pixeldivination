...
export const CartDrawer: React.FC<CartDrawerProps> = ({ isOpen, onClose }) => {
  const { items, addItem, removeItem, decreaseItem, clearCart } = useCartStore();
          ...
          >
            ...
            {checkoutState === 'success' ? (
               <div className="flex-1 flex flex-col items-center justify-center p-8 text-center animate-in fade-in zoom-in duration-300">
                  ...
                  <h3 className="text-white font-bold text-xl mb-2">支付成功</h3>
                  <p className="text-gray-400 text-sm">感谢您的购买！<br/>物品已存入您的背包。</p>
               </div>
            ) : (
              <>
                {/* Items List */}
                ...
                {cartProducts.length > 0 && (
                  <div className="p-4 border-t border-white/10 bg-[#161622]">
                    ...
                    <button 
                      ...
                      disabled={checkoutState === 'processing'}
                      className="w-full py-3 bg-gradient-to-r from-[#EAB308] to-[#F59E0B] text-black font-bold rounded-xl shadow-lg hover:brightness-110 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                    >
